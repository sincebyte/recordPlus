...
启动：
#+begin_src bash
open /Users/van/Library/Developer/Xcode/DerivedData/RecordPlusPlus-*/Build/Products/Debug/Record++.app
#+end_src
...
启动：
#+begin_src bash
open /Users/van/Library/Developer/Xcode/DerivedData/RecordPlusPlus-*/Build/Products/Release/Record++.app
#+end_src
...
#+begin_src bash
./verify.sh ~/Desktop/record_plus_plus_output_*.mov
#+end_src
...
1. 将 =output.mov= 导入 Media Pool
2. 拖入 Timeline
3. 右键片段 → =Clip Attributes → Video=，确认 =Alpha Mode= 为 =Premultiplied= 或 =Straight=
4. 在片段下方放置彩色背景，窗口外区域应显示背景色，窗口内容正常显示
